...
Item {
  ...
  Surface {
    ...
    // Same idiom as everywhere else in this shell: state recolours the outline
    // instead of introducing a second visual language.
    edge: row.failed ? Theme.get_color("danger")
        : row.connecting ? Theme.get_color("edge_alt")
        : Theme.get_color("edge")
    edge2: row.failed ? Theme.get_color("danger")
    ...
    Item {
      ...
      RowLayout {
        ...
        Text {
          ...
          color: row.failed ? Theme.get_color("danger") : Theme.get_color("muted")
        }
      }
