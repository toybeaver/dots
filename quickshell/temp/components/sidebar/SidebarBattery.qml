    ...
    NumberDisplay {
      ...
      Connections {
        ...
        function set_color(col: string) {
          display.color = Theme.get_color(col);

          // Normal keeps the resting outline; an alert replaces
          // both ends to the alert colour so it reads as a warning rather than
          // as decoration.
          if (col === "fg") {
            root.edgeColor    = Theme.get_color("edge");
            root.edgeColorAlt = Theme.get_color("edge_alt");
          } else {
            root.edgeColor    = Theme.get_color(col);
            root.edgeColorAlt = Theme.get_color(col);
          }
        }
      }
