...
//
// Drawn in QML rather than loaded from an SVG so the diamond can take a colour
// per theme. The original is two-tone (black strokes, red diamond) and that
// contrast is load-bearing — flattened to ONE colour the diamond merges into
// the strokes and the silhouette becomes a blob. This theme keeps the two tones
// as white and the background, which is the same contrast by another means.
//
// ---------------------------------------------------------------------------
...
Item {
  ...
  // Off in this theme. The diamond is painted the background colour instead,
  // which restores the mark's original reading: white strokes with a real hole
  // through the middle. hxh-neon-glass fills that hole with an accent gradient
  // because it has accents to spend; this theme does not, and a grey diamond
  // on white strokes would only mud the silhouette.
  //
  // Still switchable — set true for a gradient between `edge` and `edge_alt`.
  property bool gradientDiamond: false

  // What shows through the diamond. Matching the pill fill rather than pure
  // black would be wrong: the mark should read as cut out of the strokes, not
  // as a third shade layered on them.
  property color holeColor: Theme.get_color("bg")
  ...
  LinearGradient {
    ...
    x2: 82
    y2: 74.5
    GradientStop { position: 0.0; color: Theme.get_color("edge") }
    GradientStop { position: 1.0; color: Theme.get_color("edge_alt") }
  }
}
