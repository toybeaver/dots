...
// delegate.
//
// This surface DOES carry a blur rule (neon-glass-control-glass in
// hyprland.lua), but it covers the whole output — panel and dim both. The rule
// scopes itself to the panel alone with ignore_alpha, which is why the dim's
// alpha and the panel's must stay either side of that threshold.
