...
Item {
  ...
  BrutalSurface {
    ...
    // Hue in the block and ink on the outline, as in the light twin. The
    // shadow cannot carry over: ink on a near-black panel is invisible, so it
    // comes off the block's own hue, darkened clear of the border.
    base: row.accent
    edge: Theme.get_color("edge")
    shadowColor: Qt.darker(row.accent, 2.2)
